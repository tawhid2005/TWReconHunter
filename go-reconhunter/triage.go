...
func buildTriageFindings(endpoints []Endpoint, body string) []TriageFinding {
	...
	for _, endpoint := range endpoints {
		...
		case "auth":
			...
			details = "Authentication and login flows are strong candidates for access-control review, IDOR, and privilege escalation checks."
		case "admin":
			score += 8
			title = "Admin-related surface"
			details = "Admin paths often expose sensitive functionality and should be reviewed first for broken access control and privilege issues."
		case "file":
			score += 6
		...
		case "api":
			score += 5
			title = "API endpoint"
			details = "API endpoints often expose internal logic and parameter handling issues, especially with ID-like parameters."
		}
